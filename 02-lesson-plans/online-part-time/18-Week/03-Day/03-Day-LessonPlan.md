## 18.3 Lesson Plan - Riding with Mongoose (10:00 AM) <!--links--> &nbsp; [⬅️](../02-Day/02-Day-LessonPlan.md)
...
### 4. Students Do: When is Mongoose Good for You (5 mins)

* Tell students think about when they might want to use `Mongoose` versus, say, a vanilla MongoJS set-up, or MySQL with Sequelize. This will help edify the importance of the package.
...
### 6. Groups Do: Make a Model Schema (20 mins)
...
* Slack out the skeleton of `15-User-Schema` to the class, and have them work in breakout rooms. The instructions are outlined in the assignment, but their work consists of coding out a schema already outlined for them.
...
* But what about when we want more functionality then what do these schema properties offer?
...
* Open up the files in `16-Custom-Methods`, particularly `server.js`, `usermodel.js` and `public/index.html`.
  ...
  * Then show the students the `server.js` page where these functions are called.
    ...
    * This essentially creates a user object that isn't inserted into the database _yet_, but we can use custom methods on it to ready it's values before insertion.
...
* Then, visit `/books` to see your books listed.
      _(and yes, The Great Gatsby **is** my favorite book)_.
...
### 13. Groups Do: Finish User Model (10 mins)

* Tell the students that they'll be sent out to breakout rooms one last time. Slack out the skeleton of `19-Populate-Exercise`.
